...
#ui-datepicker-div {  z-index: 9999999 !important;}
			...
			<?php 
				...
				$unique_items = get_item_from_fms_data('sample_cards');
				$unique_s_person = get_sales_person_from_fms_data('sample_cards');
						...
						<tr class="tbale_header text-center  task_top_fix">
							...
							@foreach($taskdatas as $taskdata)
								...
								<?php }else{ ?>
											...
											<?php /* ?><br/><input type="text" name="s_person_data" value="<?php if(isset($_GET['s_person_data'])){echo $_GET['s_person_data'];}?>" class=" form-control" id="s_person_data" /><?php */ ?>
							...
							@foreach($taskdatas as $taskdata)
								...
								<td class="text-center{{ $left_fix }}">
									...
									@if($taskdata['field_type']=='timestamp')
										<a href="{{ route('samplecardshow', $pi_id) }}" target="_blank" class="pi_po_number">
...
function searchValidate(){
	...
	// openInvoiceUpdate 
	function openInvoiceUpdateSample(dtId, stpId, row_num_stage, awb_no, courier_company){
		$('.data_update').html('');
