			...
			<div class="col-md-12">
				...
				<div class="card">					
					<div class="card-body_rename card-body">
						<!-- card-body_rename -->
						<?php						
							/* $dt = get_item_list_with_price_api();
							pr($dt); die; */ 
							...
							$h_embroider = get_h_embroider_list_view_api();
							
							// hand embroider
							$h_embroidersArr = array();
							$h_embroiderDropdown='';
							$h_embroiderDropdown.='<select class="form-control text-center" name="h_embroider_list" id="h_embroider_list"><option  value="">--Select H.EMBR--</option>';
							foreach($h_embroider as $embroider){
								$h_embroidersArr[$embroider->id] = $embroider->f_name;
								$h_embroiderDropdown.='<option value='.$embroider->id.'>'.$embroider->f_name.'</option>';
							}
							$h_embroiderDropdown.='</select>';
							
							$m_embroider = get_m_embroider_list_view_api();
							$m_embroidersArr = array();
							$m_embroiderDropdown='';
							$m_embroiderDropdown.='<select class="form-control text-center" name="m_embroider_list" id="m_embroider_list"><option  value="">--Select M.EMBR--</option>';
							foreach($m_embroider as $embroider){
								$m_embroidersArr[$embroider->id] = $embroider->f_name;
								$m_embroiderDropdown.='<option value='.$embroider->id.'>'.$embroider->f_name.'</option>';
							}
							$m_embroiderDropdown.='</select>';
							
							/* END embroider list API */
							
							/* START tailor list API */
							$tailor_list_Arr = get_tailor_list_api();
							$tailorArr = array();
							$tailorArrDropdown='';
							$tailorArrDropdown.='<select class="form-control text-center" name="tailor_list" id="tailor_list"><option  value="">--Select Tailor--</option>';
							foreach($tailor_list_Arr as $tailor){
								$tailorArr[$tailor->id] = $tailor->f_name;
								$tailorArrDropdown.='<option value='.$tailor->id.'>'.$tailor->f_name.'</option>';
							}
							$tailorArrDropdown.='</select>';
							/* END tailor list API */
						...
						<form method="POST" action="{{ route('employeereport') }}" id="emp_form">
							...
							<div class="form-group row">
								...
								<div class="col-lg-2">
									...
									<select name="fms_type" class="form-control" onchange="fmsTypeCheck(this.value);">
										...
										<option value="sample_fms" <?php echo $fms_type=='sample_fms'?'selected':''; ?>>Sample FMS</option>
									</select>
								</div>
								...
								<div class="col-lg-2">
									...
									<select name="search_type" class="form-control" onchange="searchType(this.value);" id="search_type_select">
										<option value="">--Select--</option>
								...
								<div class="col-lg-2 search_list" id="fabricator_list" <?php echo $search_type!="fabricator_list"?$display_none:''; ?>>
									<label for="menu_name">{{ __('Fabricator') }}</label>
									{!! $fabDropdown !!}
								</div>
								 
								<div class="col-lg-2 search_list" id="patchers_list" <?php echo $search_type!="patchers_list"?$display_none:''; ?>>
									<label for="menu_name">{{ __("Patcher's") }}</label>
									{!! $patcherDropdown !!}
								</div>
								
								<div class="col-lg-2 search_list" id="tailor_list" <?php echo $search_type!="tailor_list"?$display_none:''; ?>>
									<label for="menu_name">{{ __("Tailor") }}</label>
									{!! $tailorArrDropdown !!}
								</div>
								
								<div class="col-lg-2 search_list" id="h_embroider_list" <?php echo $search_type!="h_embroider_list"?$display_none:''; ?>>
									<label for="menu_name">{{ __("H.EMBR") }}</label>
									{!! $h_embroiderDropdown !!}
								</div>
								
								<div class="col-lg-2 search_list" id="m_embroider_list" <?php echo $search_type!="m_embroider_list"?$display_none:''; ?>>
									<label for="menu_name">{{ __("M.EMBR") }}</label>
									{!! $m_embroiderDropdown !!}
								</div>
								...
								<div class="col-lg-1 col-md-2">
									...
									<button type="submit" class="btn btn-primary waves-effect waves-classic form-control">Search</button>
								</div>
								
								<div class="col-lg-1 col-md-2">
									<label for="search">&nbsp;</label> 
									<a href="{{ route('employeereport') }}" class="btn btn-success form-control" title="Reload">
									<i class="icon md-refresh" aria-hidden="true"></i></a>
								</div>
								...
								<tbody id="fb_table_body">
									...
									@foreach($data as $rows)
										<?php 
											...
											$fms_id = $fms_ids[$ik];
											if($fms_id!='5d0c9aafd04904203e19dc62'){
									...
									@foreach($rows as $row)
										...
										<?php 
											//pr($row); die;
											//echo $price_type."1111".$items_task_id;  pr($item_price_arr); die;
										...
										<td>{{ getFmsNameById($fms_id) }}</td>										
										<td>{{ $itemsArr[$row[$items_task_id]] }}</td>										
										<td colspan="4">Please check BOM, Is Prodiction BOM done? </td></tr>
										<?php } ?>
									@endforeach
...
<script>
	...
	function fmsTypeCheck(thisVal)
	{
		
		if(thisVal=='sample_fms'){
			$('#search_type_select').children('option[value=tailor_list]').attr('disabled', false);
			$('#search_type_select').children('option[value=h_embroider_list]').attr('disabled', false);
			$('#search_type_select').children('option[value=m_embroider_list]').attr('disabled', false);
			
			$('#search_type_select').children('option[value=fabricator_list]').attr('disabled', true);
			$('#search_type_select').children('option[value=patchers_list]').attr('disabled', true);
		}else{
			$('#search_type_select').children('option[value=tailor_list]').attr('disabled', true);
			$('#search_type_select').children('option[value=h_embroider_list]').attr('disabled', true);
			$('#search_type_select').children('option[value=m_embroider_list]').attr('disabled', true);
			
			$('#search_type_select').children('option[value=fabricator_list]').attr('disabled', false);
			$('#search_type_select').children('option[value=patchers_list]').attr('disabled', false);
		}
	}
