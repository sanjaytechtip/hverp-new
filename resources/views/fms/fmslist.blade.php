        ...
        <table class="table table-bordered table-hover table-striped" cellspacing="0" id="exampleAddRow">
          <thead>
            <tr>
              ...
              <th class="bg-info" style="color:#fff !important;">FMS Name</th>
              <th class="bg-info" style="color:#fff !important;">Actions</th>
            </tr>
          </thead>
          <tbody>
            <?php $access_fms_idArr = getCurUserField('access_fms_id'); ?>
          @foreach($fmslists as $fmslist)
          
          
          
          @if(userHasRight() || (!empty($access_fms_idArr) && in_array($fmslist['_id'], $access_fms_idArr)))
          <tr class="gradeA">
            <td>{{ $no++ }}</td>
            <td>{{$fmslist['fms_name']}}</td>
            <?php
            ...
            <a href="{{ route('fmsdata', $fmslist['_id']) }}{{ $userQuery }}" class="btn btn-primary"> <i class="icon md-eye" aria-hidden="true"></i> View</a>
