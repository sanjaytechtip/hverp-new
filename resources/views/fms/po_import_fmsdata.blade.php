							...
							@foreach($taskdatas as $taskdata)
								...
								<th class="bg-info text-white{{ $left_fix }}{{ $purple }}">
									...
									<?php }else{ ?>
											...
											?> 
											<?php if('60c33186b19624238c8ae39e' == $taskdata['_id']){
												
											$searchoption = '<br/><select name="final_destination" ><option value="">--Select--</option>';

											foreach (getOptionArray('final_destination') as $rkey=>$rval){
												if(isset($_GET['final_destination']) && ($rkey ==$_GET['final_destination'])) {$sel = ' selected=""';}else{$sel = '';}
												
												$searchoption .='<option value="'.$rkey.'" '.$sel.'>'.$rval.'</option>';
											}	
											$searchoption .='</select>'; 
											echo $searchoption;
											}
						...
						<?php /* ?><td class="text-center task_left_fix sr_no">{{ $pgNum }}</td><?php */?>
							...
							@foreach($taskdatas as $taskdata)
								...
								<td class="text-center{{ $left_fix }}">									 
									...
									@elseif($taskdata['field_type']=='final_destination')
										<?php 
											if($fmsdata['final_destination']!=''){
												echo getOptionValueByKeyValue('final_destination', $fmsdata['final_destination']);
											}
										?>
									@else
							...
							if(array_key_exists($stepsdata['_id'], $fmsdata)) {
								...
								$import_customs = '';
								if(array_key_exists('process',$stepsdata) && $stepsdata['process']=='import_customs'){
									$import_customs = $stepsdata['process'];
								}
								
								$unload_goods = '';
								if(array_key_exists('process',$stepsdata) && $stepsdata['process']=='unload_goods'){
									$unload_goods = $stepsdata['process'];
								}
								
								$if_the_quality_is_fail = '';
								if(array_key_exists('process',$stepsdata) && $stepsdata['process']=='if_the_quality_is_fail'){
									$if_the_quality_is_fail = $stepsdata['process'];
								}
								
								$step19_20_21='';
								if(array_key_exists('process', $stepsdata) && ($stepsdata['process']=='book_import' || $stepsdata['process']=='import_summary' || $stepsdata['process']=='submit_the_bill')){
									$step19_20_21=1;
								}
								$get_security_deposit='';
								if(array_key_exists('process', $stepsdata) && ($stepsdata['process']=='get_security_deposit')){
									$get_security_deposit=$stepsdata['process'];
								}
									
							if (strtotime($plane_date)>strtotime(0) && array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $unload_goods!='unload_goods' && $if_the_quality_is_fail!='if_the_quality_is_fail' && $step19_20_21!=1 && $get_security_deposit!='get_security_deposit' && $do_collect!='do_collection' && $import_customs!='import_customs' || $stepsdata['fms_when_type']==12)
							{
									$planeAlert='';
								...
								<?php 
										...
										<?php	
												...
												if(array_key_exists('process', $stepsdata) && ($stepsdata['process']=='ss_to_be_received')){
										...
										//$stock_status
											...
											<?php }elseif(array_key_exists('process', $stepsdata) && ($stepsdata['process']=='final_vessel_sailing')){ 
												...
												if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $fmsdata['etd']!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
														UpdateplannedActual($fms_id, $fmsdata['_id'], '5ee34aef0f0e7502dc003d2d', $fmsdata['etd']);
												}
												...
												$pi_actual='';
													...
													echo '<td class="text-center" id="data_step_pl_'.$stepsdata['_id'].'_row'. $row_num.'">';
														...
														if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $part_shipped_planned!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
															UpdateplannedActual($fms_id, $fmsdata['_id'], '5ee767640f0e751c0c003313', $part_shipped_planned);
												...
												if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $ss_to_be_received_pl!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
													UpdateplannedActual($fms_id, $fmsdata['_id'], '5ee8a0040f0e7507780076e8', $ss_to_be_received_pl);
													
												}
												
												if($fmsdata[$stepsdata['_id']]['planed']==''){
													$ss_to_be_received_pl=$ss_to_be_received_pl;
												}else{
													$ss_to_be_received_pl=$fmsdata[$stepsdata['_id']]['planed'];
												}
													...
													echo changeDateToDmyHi($ss_to_be_received_pl);
													...
													if($order_status!='Cancelled' && $order_status!='' && $final_vessel_sailing!=''){
														...
														if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $s13Pl!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
															UpdateplannedActual($fms_id, $fmsdata['_id'], '5ee8a0040f0e7507780076ea', $s13Pl);
														...
														if($fmsdata[$stepsdata['_id']]['planed']==''){
															...
															$s13Pl=$fmsdata[$stepsdata['_id']]['planed'];
														} 
													...
													echo changeDateToDmyHi($s13Pl);
													
													$dateHtml =  dateCompare($s13Pl, $fmsdata[$stepsdata['_id']]['actual']);
													...
													if($order_status!='Cancelled' && $import_customs_act!=''){
														...
														if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $s14Pl_main!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
															UpdateplannedActual($fms_id, $fmsdata['_id'], '5ee8a0040f0e7507780076eb', $s14Pl_main);
															
														}
														
														if($fmsdata[$stepsdata['_id']]['planed']==''){
															$s14Pl_main=$s14Pl_main;
														}else{
															$s14Pl_main=$fmsdata[$stepsdata['_id']]['planed'];
														}
															
														$s14Pl = changeDateToDmyHi($s14Pl_main);
														echo '<td class="text-center" id="data_step_pl_'.$stepsdata['_id'].'_row'. $row_num.'">'.$s14Pl.'</td>';
														
														$dateHtml =  dateCompare($s14Pl_main, $fmsdata[$stepsdata['_id']]['actual']);
													...
													if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $s17Pl_main!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
														UpdateplannedActual($fms_id, $fmsdata['_id'], '5eeb11900f0e750a40003668', $s17Pl_main);
														
													}
													
													if($fmsdata[$stepsdata['_id']]['planed']==''){
														$s17Pl_main=$s17Pl_main;
													}else{
														$s17Pl_main=$fmsdata[$stepsdata['_id']]['planed'];
													}
													...
													if($fmsdata[$stepsdata['_id']]['actual']!=''){
														$dateHtml =  dateCompare($s17Pl_main,$fmsdata[$stepsdata['_id']]['actual']);
													}
													echo '<td class="text-center" id="step_actual_'.$stepsdata['_id'].'_row'. $row_num.'" style="background:'.$dateHtml['bgcolor'].';">'; 
												...
												// step = 19, 20, 21
													...
													if($order_status!='Cancelled' && $unload_goods_act!=''){
														...
														if($unload_goods_act!=''){
															...
															if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $planned_dt_main!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
																UpdateplannedActual($fms_id, $fmsdata['_id'], $stepsdata['_id'], $planned_dt_main);
															}
															
															if($fmsdata[$stepsdata['_id']]['actual']!=''){
																$dateHtml =  dateCompare($planned_dt_main,$fmsdata[$stepsdata['_id']]['actual']);
															}
													...
													if($order_status!='Cancelled' && $import_customs_act!='' && $security_deposit_dropdown=='Yes'){
														...
														if(array_key_exists('planed',$fmsdata[$stepsdata['_id']]) && $s22Pl_main!='' && $fmsdata[$stepsdata['_id']]['planed']==''){
															UpdateplannedActual($fms_id, $fmsdata['_id'], $stepsdata['_id'], $s22Pl_main);
														}
														
														if($fmsdata[$stepsdata['_id']]['actual']!=''){
															$dateHtml =  dateCompare($s22Pl_main,$fmsdata[$stepsdata['_id']]['actual']);
														}
