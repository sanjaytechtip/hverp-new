...
<?php
				...
				$masterListArr = array();
				$masterDropdown='';
				$masterDropdown.='<select class="form-control text-center" name="master_list" id="masterOptionList" style="width:160px;margin-top: 7px;margin-right: 5px;" onchange="getMasterRecord(this.value);"><option value="">--Select Master--</option>';
				foreach($masterLists as $masterList){
					$masterListArr[$masterList->emp_id] = $masterList->f_name;
					$masterDropdown.='<option value='.$masterList->emp_id.'>'.$masterList->f_name.'</option>';
				}
				...
				$mainItemArr = $itemsArrAll['mainItemArr']; */
						...
						<nav>
							<ul class="pagination">
								...
								<li class="page-item<?php echo $active;?>">
									<a class="page-link" href="{{ $fmsdatas_pg['path'] }}?page={{ $pg }}&fab_id={{ $fab_id }}&order_month={{ $order_range }}&item_id={{ $item_id }}&order_number={{ $orderNum }}&completed={{ $completed }}">{{ $pg }}</a>
								</li>
								<?php } ?>
							</ul>
						</nav>
				...
				<div class="pop_form row">
					<div class="col-md-12">
						<form action="" method="POSt" onsubmit="event.preventDefault(); saveFabricator();" id="fabricatorForm">
							...
							<input type="hidden" name="data_o_id" id="data_o_id" value="">
