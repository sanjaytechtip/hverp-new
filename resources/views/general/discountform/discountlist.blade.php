...
@section('pageTitle', 'Sale Discount List')
@section('pagecontent')
<div class="page">
    ...
    <div class="new-design-inner">
      <div class="new-header-div-fot-btn">
        <div class="page-header">
          <h1 class="page-title">Sale Discount List</h1>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item">Sale Discount List</li>
          </ol>
        </div>
        <div class="payment-heading-box1">
        <form method="POST" action="{{url('admin/discountlist')}}" class="custom-inline-form">
          @csrf
          <div class="form-box">
          ...
          <div class="form-box">
            <input type="text" class="form-control" autocomplete="off" placeholder="Brand Name" name="brand_name" value="{{$brand_name}}">
          </div>
          ...
          <div class="form-box form-box-submit"> <a href="{{url('admin/discountlist')}}" class="btn btn-primary waves-effect waves-classic" title="Reload" name="reload"><i class="icon md-refresh" aria-hidden="true"></i></a></div>
          <div class="form-box form-box-submit"> @if(userMenuAccessRight(getUserAccess(),'discount_form_create')) <a class="btn btn-primary waves-effect waves-classic" style="float:right;" href="{{ route('discount_form_create') }}"> <i class="icon md-plus" aria-hidden="true"></i> Add</a> @endif </div>
          </div>
        </form>
      </div>
          ...
          <div class="panel-body"> @if(!empty($datas))
            <table class="table table-bordered t1">
              ...
              <tr>
                ...
                <td style="text-align:center;" class="action"><div class="action-wrap"> 
                    <!-- <label class="radio-btn-action"><input type="radio" value="{{$data['id']}}" class="action_data" name="action_data" aria-required="true"></label> --> 
                    <span id="action-{{$data['id']}}" class="action-data-show"> <a title="Edit" href="javascript:void(0);" data-toggle="modal" data-target="#myModal-{{$data['id']}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-edit" aria-hidden="true"></i></a>
                      ...
                      <div class="modal-dialog modal-sm"> 
                        ...
                        <div class="modal-content">
                          <div class="modal-header">
                            ...
                            <h4 class="modal-title">Sale Discount Update</h4>
                          </div>
                    ...
                    <span id="del_{{$data['id']}}"><a title="Delete" onclick="return confirm('Are you sure you want to delete?');" href="{{url('admin/delete_discount/'.$data['id'])}}" class="btn btn-pure btn-danger icon waves-effect waves-classic"><i class="icon md-delete" aria-hidden="true"></i></a></span> </span> </div></td>
              </tr>
              @php $i++ @endphp
        ...
        $(document).on('click','.update-net-rate',function(){
            ...
            var date_update = "{{date('d-m-Y')}}";
                ...
                if(id!='' && date_rate!='' && net_rate!=''){
                    $.ajax({
                    url: "{{ url('admin/discount_data_update') }}" ,
                    type: "POST",
