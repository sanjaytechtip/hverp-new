...
@section('pageTitle', 'Sale Order List')
@section('pagecontent')
    ...
    <div class="new-design-inner">
      <div class="new-header-div-fot-btn search-design-data">
        <div class="page-header">
          <h1 class="page-title">Sale Order List</h1>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item">Sale Order List</li>
          </ol>
        </div>
        <div class="payment-heading-box1">
        <form method="POST" action="{{url('admin/saleorder-list')}}" class="custom-inline-form">
          @csrf
          <div class="form-box">
              <input class="form-control" type="text" name="saleorder_no" value="{{$request->saleorder_no}}" autocomplete="off" placeholder="SO. No">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" name="saleorder_priority" value="{{$request->saleorder_priority}}" autocomplete="off" placeholder="SO. Priority">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" id="saleorder_date" name="saleorder_date" value="{{$request->saleorder_date}}" autocomplete="off" placeholder="SO. Date">
            </div>
            <div class="form-box">
          ...
          <div class="form-box"><select class="required form-control" id="customer_name" name="customer_name" data-plugin="select2" data-placeholder="Type Customer" data-minimum-input-length="1">
            ...
            <div class="form-box">
              <select class="required form-control" name="saleorder_status" data-placeholder="Status" data-minimum-input-length="1">
                <option value="">-- Select --</option>
                <option @if($request->saleorder_status=='Saved') selected @endif value="Saved">Saved</option>
                <option @if($request->saleorder_status=='Pending Approval') selected @endif value="Pending Approval">Pending Approval</option>
                <option @if($request->saleorder_status=='Approved') selected @endif value="Approved">Approved</option>
            </select>
            </div>
          ...
          <div class="form-box form-box-submit"><a href="{{route('saleorder_list')}}" class="btn btn-primary waves-effect waves-classic" title="Reload" name="reload"><i class="icon md-refresh" aria-hidden="true"></i></a></div><div class="form-box  form-box-submit">
              <a class="btn btn-primary waves-effect waves-classic" style="float:right;" href="{{ route('saleordercreate') }}"> <i class="icon md-plus" aria-hidden="true"></i> Add</a>
              </div>
          </div>
        </form>
      </div>
      <div class="page-content">
        ...
        <div class="panel">
          <div class="panel-body"> @if(!empty($saleorderlist))
            <table class="table table-bordered t1 custom-lineheight-table">
              <thead>
                <tr>
              <th class="bg-info sort ta" id="ticket_number" style="color:#fff !important;">SO. No</th>
                ...
                <th class="bg-info sort ta" id="customer" style="color:#fff !important;">SO. Date</th>
                ...
                <th class="bg-info" style="color:#fff !important;">Action</th></tr>
                  </thead>
              <tbody id="fb_table_body">
              @php
              $i=($saleorderlist->currentpage()-1)* $saleorderlist->perpage();
              $user_data = getAllusersDataList();
              $cus_data = getAllBuyerData();
              @endphp
              @foreach($saleorderlist as $data)
              <tr>
                <td>{{ $data['saleorder_no'] }}</td>
                <td> {{ucfirst($data['saleorder_priority'])}}</td>
                <td class="white-space-normal custom-white-space-normal"> {{ $cus_data[$data['customer_name']] }} </td>
                <td> {{ globalDateformatNet($data['saleorder_date'])}} </td>
                <td> {{ $data['saleorder_ref_no']}} </td>
				<td> {{ $data['saleorder_grand_total']}}</td>
                <td> {{ $user_data[$data['created_by']] }}</td>
				<td> {{ $user_data[$data['approved_by']] }}</td>
                <td> {{ $data['saleorder_status']}} </td>
                <td style="text-align:center;" class="action">
                    <div class="action-wrap">
                    <a title="Edit" href="{{url('admin/saleorder-edit/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-edit" aria-hidden="true"></i></a> 
                    <a title="View" href="{{url('admin/saleorder-view/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-eye" aria-hidden="true"></i></a> 
                    <a title="Print" target="_blank" href="{{url('admin/saleorder-print/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-print" aria-hidden="true"></i></a>
                    <a title="PDF Download" href="{{url('admin/saleorder-pdf/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-collection-pdf" aria-hidden="true"></i></a>
                    <span id="del_{{$data['id']}}">
                        <a title="Delete" onclick="return confirm('Are you sure you want to delete?');" href="{{url('admin/delete_saleorder/'.$data['id'])}}" class="btn btn-pure btn-danger icon waves-effect waves-classic"><i class="icon md-delete" aria-hidden="true"></i></a>
                        </span>
                    </div>
                    </td>
              </tr>
          ...
          <div class="pagination-wrap">{{ $saleorderlist->links() }}</div>
        </div>
      </div>
    </div>
  </div>
