...
<div class="page creat-fms-box">
  ...
  <div class="page-content container-fluid">
    <div class="row justify-content-center"> @if (\Session::has('success'))
      ...
      @endif
      <div class="col-md-12">
        <form class="" action="{{ url('admin/saleordercreate') }}" id='invoiceform' method="post" autocomplete="on" onSubmit="return validate();">
          @csrf
          <div class="card">
          <div class="card-body">
            <div class="row form-new-box-wrap new-quotationcreate new-fix-quotationcreate">
              ...
              <div class="col-lg-4 col-md-4 col-sm-12 ">
                ...
                <div class="row form-two-box">
                  ...
                  <div class="col-lg-6 col-md-6 col-sm-12 ">
                    <label>Satus</label>
                    <select name="quotation_status" id="quotation_status" required="" class="form-control" aria-required="true" disabled="">
              ...
              <div class="col-lg-4 col-md-4 col-sm-12 recurring-order">
                ...
                <div class="check-box-wrap"> Recurring Order? <small>Recurring Orders re-occur on certain days or dates of every week, month or year. </small> </div>
                </label>
               
                    <div class="row form-three-box">
                      <div class="col-lg-4 col-md-4 col-sm-12 recurring_type" style="display:none;">
                        <select name="recurring_type" id="recurring_type" required class="form-control">
                          <option value="">Select</option>                        
    @foreach(getRecurringData() as $key=>$recurring_data)    
                          <option value="{{$key}}">{{$recurring_data}}</option>                          
    @endforeach    
                        </select>
                      </div>
                      <div class="row col" id="result-recurring-type"> </div>
                    </div>
			  ...
			  <div class="col-lg-4 col-md-4 col-sm-12 recurring-order">
                <label class="col-sm-12 col-sm-12 control-label checkbox-wrap">
                <input type="checkbox" name="is_scheduled" id="is_scheduled" value="2">
            ...
            <div class="purchaseorder-box quotationcreate-box-wrap">
              <label for="contact"><strong>Item Details</strong> <a data-toggle="modal" class="search-quotation-pop-up small-btn  btn-sm btn-primary" href="javascript:void(0);">Search Quotations</a></label>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Select Quotations</h4>
      </div>
        ...
        <form method="POST" id="reloadform_qu" action="" class="custom-inline-form item-form-apply-qu">
          ...
          <div class="new-form-box">
            <div class="form-wrap small-box">
              ...
              <input class="form-control" type="text" id="quotation_no" name="quotation_no" value="{{@$quotation_no}}" autocomplete="off" placeholder="">
            </div>
        ...
        $(function(){
            $(document).on('click','.data-append',function(){
               //var quotation_id = $(this).val();
                var check_schedule_order =  $("input[name='is_scheduled']:checked").val();
                if (check_schedule_order === undefined || check_schedule_order === null) {
                var schedule_order = 0;
                }else{
                var schedule_order = check_schedule_order;  
                }
                ...
                //alert(quotation_id);
                   ...
                   var rowCount_d_val = $('#tbl_length').val();
                    ...
                    $.ajax({
                    url: "{{ url('admin/ajaxSalesItems') }}" ,
                    type: "POST",
                    data: {quotation_id:quotation_id,purchase_order_row:purchase_order_row,rowCount_d_val:rowCount_d_val,schedule_order:schedule_order},
                    success: function( response ) {
                    //alert(response); return false;
                    if(response!='')
                    {
                    ...
                    //alert(purchase_order_row+'---'+purchase_order_add_row);
                    ...
                    $('#purchase-order-row').val(1);
                     ...
                     $('#saleorder_contact_name').val(returnedData.saleorder_contact_name);
                     $('#saleorder_contact_department').val(returnedData.saleorder_contact_department);
                     $('#saleorder_contact_phone').val(returnedData.saleorder_contact_phone);
                     $('#saleorder_contact_email').val(returnedData.saleorder_contact_email);
        ...
        $(function(){
           $(document).on('click','.search-quotation-pop-up',function(){
              ...
              if(customer_id!='')
              {
                    $.ajax({
                    type: "GET",
                    url: "{{ url('admin/ajaxSearchQuotation')}}/"+customer_id,
                    data: {customer_id:customer_id},
                    success: function(msg){
                    //alert(msg); return false;
                    if(msg!='')
                    ...
                    var quotation_no = $("#quotation_no").val();
                    $.ajax({
                    url: "{{ url('admin/purchase_net_rate_search_qu') }}" ,
                    type: "POST",
    ...
    $(function(){
        ...
        $(document).on('click','.order_search_qu',function(){
                ...
                var formulario =  $(".item-form-apply-qu");
                    ...
                    url: "{{ url('admin/purchase_net_rate_search_qu') }}" ,
                    type: "POST",
                    data: {quotation_no:quotation_no,customer_id:customer_id},
                    success: function( response ) {
                    //alert(response); return false;
...
<script>
    $('input[name="is_recurring"]').click(function(){
      ...
      if(tbl_length>=1)
      {
        var verify = confirm("If you move to (Normal/Recurring/Scheduled) Order, then below items should be removed. Are you sure you want to remove this items?");
        if(verify){
          $('#editableTable1 >tbody >tr').remove(); 
          var rowCount = $('#tbl_length').val(0);
        }else{
          return false;
        }
      }
   if ($("input[name='is_recurring']:checked" ).val() === "1") {
            ...
            $('#is_scheduled').prop('checked',false);
      } else {
        $('.recurring_type').hide();  
