...
@section('pageTitle', 'DSS Live List')
@section('pagecontent')
<div class="page">
	<div class="page-header">
		<h1 class="page-title">DSS Live List</h1>
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
			<li class="breadcrumb-item"><a href="{{ route('stockpurchaselist') }}">Stock Purchase List</a></li>
			<li class="breadcrumb-item">DSS Live List</li>
		</ol>
	</div>
			...
			<div class="panel-body">
				...
				<table class="table table-bordered table-hover table-striped dataTable no-footer purchase-data-table">
					<thead>
						...
						<th class="bg-info" style="color:#fff !important;">Date | <button type="submit" class="btn btn-success">Search</button> <a href="{{ route('dsslivelist') }}" > <i class="icon md-refresh" aria-hidden="true"></i> Reset </a></th>
					</thead>
					<tbody id="fb_table_body">
				    	@foreach($data as $row)
							<tr>
								...
								<td>
								<?php 
									if(array_key_exists('po_no',$row)){
										echo $row['po_no'];
									}elseif(array_key_exists('pi_no',$row)){
										echo $row['pi_no'];
									}
								?>
								</td>
