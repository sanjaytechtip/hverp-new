...
@section('pageTitle', 'Purchase Net Rate List')
@section('pagecontent')
    ...
    <div class="new-design-inner">
      <div class="new-header-div-fot-btn">
        <div class="page-header">
          <h1 class="page-title">Purchase Net Rate List</h1>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item">Purchase Net Rate List</li>
          </ol>
        </div>
        <div class="payment-heading-box1">
        <form method="POST" action="{{url('admin/purchase-net-rate-list')}}" class="custom-inline-form">
          @csrf
          ...
          <div class="form-box form-box-submit"><a href="{{url('admin/purchase-net-rate-list')}}" class="btn btn-primary waves-effect waves-classic" title="Reload" name="reload"><i class="icon md-refresh" aria-hidden="true"></i></a></div><div class="form-box  form-box-submit">
              @if(userMenuAccessRight(getUserAccess(),'netrate_form_create'))
              <a class="btn btn-primary waves-effect waves-classic" style="float:right;" href="{{ route('purchase_netrate_form_create') }}"> <i class="icon md-plus" aria-hidden="true"></i> Add</a>
              @endif
              </div>
          </div>
        </form>
          ...
          <div class="panel-body"> @if(!empty($datas))
            <table class="table table-bordered t1">
              ...
              <th class="bg-info" style="color:#fff !important;">Sr. No</th>
                ...
                <th class="bg-info sort ta" id="customer_name" style="color:#fff !important;">Customer</th>
              ...
              <tr>
                <td>{{ $i+1 }}</td>
                <td class="white-space-normal" style="line-height:normal">{{getAllitemsById($data['product_id'])}}</td>
                <td class="white-space-normal" style="line-height:normal">{{getCustomerNamefromId($data['customer_id'])}}</td>
                ...
                <td style="text-align:center;" class="action">
                    ...
                    <h4 class="modal-title">Net Rate Update</h4>
                    </div>
                  ...
                  <div class="form-group">
                    ...
                    <span id="del_{{$data['id']}}">
                        <a title="Delete" onclick="return confirm('Are you sure you want to delete?');" href="{{url('admin/delete_purchase_netrate/'.$data['id'])}}" class="btn btn-pure btn-danger icon waves-effect waves-classic"><i class="icon md-delete" aria-hidden="true"></i></a>
                        </span>
                    </span>
                    </div>
                    </td>
        ...
        $(document).on('click','.update-net-rate',function(){
            ...
            var date_update = "{{date('d-m-Y')}}";
                ...
                if(id!='' && date_rate!='' && net_rate!=''){
                    $.ajax({
                    url: "{{ url('admin/purchase_netrate_data_update') }}" ,
                    type: "POST",
