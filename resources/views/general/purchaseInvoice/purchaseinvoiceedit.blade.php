...
@section('pageTitle', 'Edit Proforma Invoice Page')
...
<div class="page creat-fms-box">
  <div class="page-header">
    <h1 class="page-title">{{ __('Edit Proforma Invoice') }}</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
      <li class="breadcrumb-item"><a href="{{ route('invoicelisting') }}">Proforma Invoice List</a></li>
      <li class="breadcrumb-item">{{ __('Edit Proforma Invoice') }}</li>
    </ol>
	  ...
	  <div class="col-sm-4">
        ...
        <div class="col-sm-4">
          <div class="counter">
             ...
             <div class="counter-label">PI-POS-{{ $edit_purchase_invoice['financial_year'] }}-{{ $edit_purchase_invoice['po_serial_number'] }}{{ $rev_p }}{{ $rev }}</div></div>
        </div>
        <div class="col-sm-4">
          <div class="counter">
  ...
  <div class="page-content container-fluid">
    <div class="row justify-content-center"> @if (\Session::has('success'))
      ...
      <div class="col-md-12">
			...
			<?php if($partshipped!=''){ ?> 
				<form class="" id="invoiceform" action="{{ url('admin/invoiceupdate_partshipped/'.$edit_purchase_invoice['_id']) }}" method="post" autocomplete="on" onsubmit="return validate();">
				<input type="hidden" name="partshipped_no" value="<?php echo $partshipped; ?>">
			<?php }else{ ?>
                  ...
                  <div class="row" style="margin:0;">
					  ...
					  ?>
					   ...
					   <div class="col-lg-6 col-md-6 col-sm-12">
						  <div class="contact-form bs-example" style="margin-top:10px;">
							...
							<?php /*?><input type="text" name="customer_address" id="customer_address" value="{{ $edit_purchase_invoice['customer_address'] }}" class="required form-control auto-fill-field"> <?php */?>
                    ...
                    <div class="contact-form bs-example contact-heading col-lg-12 col-md-12 ">
                      ...
                      <div class="contact-form bs-example col-lg-3 col-md-4" style="margin-top:10px;">
						...
						{!! getUserByDesignation(SALES_AGENT,'sales_agent','---Select---',$edit_purchase_invoice['sales_agent'], 'id="sales_agent"') !!}
                       </div>
                      <div class="contact-form bs-example col-lg-3 col-md-4" style="margin-top:10px;">
                      ...
                      <div class="contact-form bs-example col-lg-3 col-md-4" style="margin-top:10px;">
						...
						<div class="contact-form bs-example col-lg-3 col-md-4" style="margin-top:10px;">
							...
							<input type="text" name="proforma_invoice_date" value="{{ date('Y-m-d',strtotime($edit_purchase_invoice['proforma_invoice_date'])) }}" class="form-control proforma_invoice_date" required />
						</div>
						<?php */ ?>
						
						<input type="hidden" name="proforma_invoice_date" value="{{ date('Y-m-d',strtotime($edit_purchase_invoice['proforma_invoice_date'])) }}"/>
					   ...
					   ?>
                        <tr id="data-row-{{ $i }}">
                          <td data-text="Item">
							...
							<?php /*?>
                            <select required class="form-control width-full item" cus="{{$i}}" rel="item" id="tmp_id_item_{{$i}}" name="row[{{$i}}][item]" data-plugin="select2" data-placeholder="Type name" data-minimum-input-length="2">
                            <option value="">-- Select --</option>
                          ...
                          <td data-text="Colour">
								...
								<?php /*?>
							  <select required class="form-control width-full" cus="{{$i}}" rel="colour" id="tmp_id_colour_{{$i}}" name="row[{{$i}}][colour]" data-plugin="select2" data-placeholder="Type color" data-minimum-input-length="2" rel="colour">
							   <option value="">--Select--</option>
							   {!! colorDataDetails($rowData['item'],$rowData['colour']) !!}
							   </select>
							   <input type="text" required class="form-control width-full color-field" cus="{{$i}}" rel="colour" id="tmp_id_colour_{{$i}}" name="row[{{$i}}][colour]" rel="colour" value="{{ $rowData['colour'] }}">
								<?php */?>
                          ...
                          <td>
						   @if($data_status!=1)
						  <a href="javascript:void(0);" onclick="return removeRow({{ $i }});" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row waves-effect waves-classic" data-toggle="tooltip" data-original-title="Remove"><i class="icon md-delete" aria-hidden="true"></i></a>
							@endif
					  </td>
                    ...
                    <div class="add-row">
                      <div class="add-left">
                        <div class="dropdown two-button">
                          <button type="button" class="btn btn-primary add-new-line" cus="1"  @if($data_status==1) disabled @endif >+ Add a new line</button>
                      ...
                      <!--<div class="row address-wrap">
                        <div class="col-lg-9 col-md-9 col-sm-12">
                          <p>Rupees {{ numberTowords($edit_purchase_invoice['total']) }} Only.</p>
                          <p> 1. The company has the right to revise the prices in cases of change in customs duty, any other statutory levies.</p>
                          <p> 2. The company shall not be responsible for delay/ cancellation of orders in case of force majeure conditions.</p>
                          <p> 3. For cash/ advance payment terms, the goods will be kept on hold up to 3 days, after which in case of non-receipt of payment, the PI shall be cancelled.</p>
                          <p> 4. Shipment Sample yardage upto 2mtr will be Free of Charge. Additional quantity shall be charged 40% extra or on customers courier account.</p>
                          <p> 5. In case the garment style involves combination of various colours, it must be communicated at the time of order placement. Also the buyer must check the suitability wrt Colour Migration, at sampling stage and from the shipment sample yardage from bulk fabric, as even for Approved/ Pass FPT this problem might be faced for certain colour combinations. </p>
                          <br />
                          <p>Kindly acknowledge your acceptance and send us a signed copy of the PI</p>
                          <p><strong>BANK DETAILS:</strong></p>
                          <p><strong>Pashupati Overseas LLP</strong></p>
                          <p><strong>Bank Name: Kotak Mahindra Bank Ltd.</strong></p>
                          <p><strong>A/c No:  2412056651</strong></p>
                          <p><strong>IFSC Code:KKBK0004634</strong></p>
                          <p><strong>Swift code : KKBKINBBCPC (FOR INTERNATIONAL BUYER)</strong></p>
                          <p><strong>Branch Address: Netaji Subhash Place, Pitampura, Delhi</strong></p>
                        </div>
                      </div>-->
                    </div>
                    <div class="save-row">
					  ...
					  <input type="hidden" name="financial_year" value="{{ $edit_purchase_invoice['financial_year'] }}" />
                        <button type="submit" name="approve" value="approve" class="btn btn-success approve">Submit</button>
