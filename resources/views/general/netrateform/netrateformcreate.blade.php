...
@section('pageTitle', 'Add Sale Net Rate Form')
@section('pagecontent')
...
<div class="page formbuilder">
  <div class="page-header">
    <h1 class="page-title">Add Sale Net Rate Form</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
      <li class="breadcrumb-item"><a href="{{ route('netratelist') }}">Sale Net Rate : List</a></li>
      <li class="breadcrumb-item">Add Sale Net Rate Form</li>
    </ol>
  </div>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-body_rename card-body">
            <h2 class="form-box form-box-heading">Sale Net Rate Form</h2>
            <div class="custom-form-box">
              <form class="custom-form" onSubmit="return checkDiscountPurchase();" action="{{url('admin/add_netrate_form')}}" id="task_form" method="POST" enctype="multipart/form-data" novalidate="novalidate">
                @csrf
                <div class="form-box rows  line_repeat_1">
                  <div class="form-box col2">
                    ...
                    <div class="inline-radio-box inline-radio single-label">
                      ...
                      <label>
                        <input type="radio" value="2" class=" block_unblock " id="" required="" name="option" aria-required="true">Customer Product Wise</label>
                  ...
                  <div class="form-box col2">
                    <label for="discription" class="custom-dis-box"><span>Net Rate<span class="required" aria-required="true">*</span></span><span id="net-rate-discount"></span></label>
                    <input type="number" class="form-control net_rate" autocomplete="off" required name="net_rate" placeholder="Net Rate">
                  </div>
                  <div class="form-box col2">
                    <label for="discription">Quantity</label>
                    <input type="number" readonly class="form-control qty" autocomplete="off" placeholder="Quantity">
                  </div>
                </div>
...
<div class="modal fade" id="myModal" role="dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body search-data-items">
          ...
          <table class="table table-bordered t1">
              ...
              <tr>
                <td style="text-align:center;" class="white-space-normal"><input stock="{{$user['stock']}}" id="{{$user['id']}}" value="{{$user['vendor_sku']}}-{{$user['name']}}-{{$user['hsn_code']}}-{{$user['grade']}}-{{$user['brand']}}-{{$user['packing_name']}}-{{$user['list_price']}}-{{$user['mrp']}}-{{$user['net_rate']}}-{{$user['stock']}}" name="apply-radio" type="radio"></td>
  ...
  <input type="hidden" id="pur_net_rate">
@endsection
...
<script>
    $(document).ready(function(){
        $(".btn-apply").click(function(){
           ...
           $('#product_id').val(id);
           $('.qty').val( $("input[name='apply-radio']:checked").attr('stock'));
        });
    });
</script> 
<script>
...
<script>
  $(function(){
      $(document).on('change click','.btn-apply,#customer_name',function(){
             ...
             if(product_id!='')
             {
              ...
              $.ajax({
                    url: "{{ url('admin/get_purchase_net_rate_data') }}" ,
                    type: "POST",
                    data: {product_id:product_id,customer_name:customer_name},
                    success: function( response ) {
                    //console.log(response); return false;
                    if(response!='')
                    {
                    $('#net-rate-discount').html('Purchase Net Rate : '+ response);
                    $('#pur_net_rate').val(response);
                    }else{
                    $('#net-rate-discount').html(''); 
                    $('#pur_net_rate').val('');
                    }
                    }
                    }); 
...
<script>
  function checkDiscountPurchase(){
    var pur_net_rate = parseInt($('#pur_net_rate').val());
    var net_rate = parseInt($('.net_rate').val());
    //console.log(pur_net_rate+"-"+net_rate); return false;
    if(pur_net_rate!='' && net_rate!='' && net_rate<pur_net_rate)
    {
     alert("Net Rate should be apply greater or equal to purchase net rate.");
     return false;
    }
    else if(pur_net_rate=='' || isNaN(pur_net_rate) && (net_rate=='' || net_rate!='' || isNaN(net_rate)))
    {
    alert("No purchase net rate found. Please choose anther one.");
     return false;
    }
    else
    {
