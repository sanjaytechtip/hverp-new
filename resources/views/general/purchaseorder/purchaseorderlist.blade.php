...
@section('pageTitle', 'Purchase Order List')
@section('pagecontent')
    ...
    <div class="new-design-inner">
      <div class="new-header-div-fot-btn search-design-data">
        <div class="page-header">
          <h1 class="page-title">Purchase Order List</h1>
          <ol class="breadcrumb">
        ...
        <form method="POST" action="{{url('admin/purchaseorder-list')}}" class="custom-inline-form">
          @csrf
          <div class="form-box">
              <input class="form-control" type="text" name="purchaseorder_no" value="{{$request->purchaseorder_no}}" autocomplete="off" placeholder="PO. No">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" name="purchaseorder_priority" value="{{$request->purchaseorder_priority}}" autocomplete="off" placeholder="PO. Priority">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" id="purchaseorder_date" name="purchaseorder_date" value="{{$request->purchaseorder_date}}" autocomplete="off" placeholder="PO. Date">
            </div>
            <div class="form-box">
            ...
            <div class="form-box">
              <select class="required form-control" id="approved_by" name="approved_by" data-plugin="select2" data-placeholder="Approved By" data-minimum-input-length="1">
            <option value="">-- Select --</option>
            </select>
            </div>
          <div class="form-box"><select class="required form-control" id="customer_name" name="customer_name" data-plugin="select2" data-placeholder="Type Customer" data-minimum-input-length="1">
            ...
            <div class="form-box">
              <select class="required form-control" name="purchaseorder_status" data-placeholder="Status" data-minimum-input-length="1">
                <option value="">-- Select --</option>
                <option @if($request->purchaseorder_status=='Saved') selected @endif value="Saved">Saved</option>
                <option @if($request->purchaseorder_status=='Pending Approval') selected @endif value="Pending Approval">Pending Approval</option>
                <option @if($request->purchaseorder_status=='Approved') selected @endif value="Approved">Approved</option>
            </select>
            </div>
          ...
          <div class="form-box form-box-submit"><a href="{{route('purchaseorder_list')}}" class="btn btn-primary waves-effect waves-classic" title="Reload" name="reload"><i class="icon md-refresh" aria-hidden="true"></i></a></div><div class="form-box  form-box-submit">
              <a class="btn btn-primary waves-effect waves-classic" style="float:right;" href="{{ route('purchaseordercreate') }}"> <i class="icon md-plus" aria-hidden="true"></i> Add</a>
              </div>
          </div>
        </form>
      </div>
      <div class="page-content">
        ...
        <div class="panel">
          <div class="panel-body"> @if(!empty($purchaseorderlist))
            <table class="table table-bordered t1 custom-lineheight-table">
              <thead>
              <th class="bg-info sort ta" id="ticket_number" style="color:#fff !important;">PO. No</th>
                <th class="bg-info sort ta" id="location" style="color:#fff !important;">Priority</th>
				<th class="bg-info sort ta" id="department" style="color:#fff !important;">Vendor</th>
                <th class="bg-info sort ta" id="department" style="color:#fff !important;">Customer</th>
                <th class="bg-info sort ta" id="customer" style="color:#fff !important;">PO. Date</th>
              ...
              @php
              $i=($purchaseorderlist->currentpage()-1)* $purchaseorderlist->perpage();
              $user_data = getAllusersDataList();
              $cus_data = getAllBuyerData();
              $ven_data = getAllVendorData();
              @endphp
              @foreach($purchaseorderlist as $data)
			  <?php $data = (array) $data;?>
              <tr @if($data['purchaseorder_status']=='Completed') class="custom-table-success" @endif>
                <td>{{ $data['purchaseorder_no'] }}</td>
                <td> {{ucfirst($data['purchaseorder_priority'])}}</td>
                <td class="white-space-normal custom-white-space-normal"> {{ $ven_data[$data['vendor_name']] }} </td>
				
                <td class="white-space-normal custom-white-space-normal"> {{ $cus_data[$data['customer_name']] }} </td>
                <td> {{ globalDateformatNet($data['purchaseorder_date'])}} </td>
				<td> {{ $data['purchaseorder_grand_total']}}</td>
                <td> {{ $user_data[$data['created_by']] }}</td>
				<td> {{ $user_data[$data['approved_by']] }}</td>
                <td> {{ $data['purchaseorder_status']}} </td>
                <td style="text-align:center;" class="action">
                    <div class="action-wrap">
                    <a title="Edit" href="{{url('admin/purchaseorder-edit/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-edit" aria-hidden="true"></i></a> 
                    <a title="View" href="{{url('admin/purchaseorder-view/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-eye" aria-hidden="true"></i></a> 
                    <a title="Print" target="_blank" href="{{url('admin/purchaseorder-print/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-print" aria-hidden="true"></i></a>
                    <a title="PDF Download" href="{{url('admin/purchaseorder-pdf/'.$data['id'])}}" class="btn btn-pure btn-success icon  waves-effect waves-classic waves-effect waves-classic"><i class="icon md-collection-pdf" aria-hidden="true"></i></a>
                    <span id="del_{{$data['id']}}">
                        <a title="Delete" onclick="return confirm('Are you sure you want to delete?');" href="{{url('admin/delete_purchaseorder/'.$data['id'])}}" class="btn btn-pure btn-danger icon waves-effect waves-classic"><i class="icon md-delete" aria-hidden="true"></i></a>
                        </span>
                    </div>
                    </td>
              </tr>
          ...
          <div class="pagination-wrap">{{ $purchaseorderlist->links() }}</div>
        </div>
      </div>
    </div>
  </div>
