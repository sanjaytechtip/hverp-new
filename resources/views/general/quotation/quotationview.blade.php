...
@section('pageTitle', 'View Quotation')
@section('pagecontent')
...
<div class="page creat-fms-box">
  <div class="page-header">
    <h1 class="page-title">{{ __('View Quotation') }}</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
      <li class="breadcrumb-item"><a href="{{ route('quotation_list') }}">Quotation : List</a></li>
      <li class="breadcrumb-item">{{ __('View Quotation') }}</li>
    </ol>
    <div class="page-header-actions custom-page-heading">
      <div class="row no-space w-400">
        ...
        @if(!empty($quotation_details_revised))
        <div class="col-sm-6">
          <div class="counter">
          <span class="counter-number font-weight-medium">Revise Quotation</span>
            <div class="counter-label">
            <select class="form-control" id="quotation_revised">
            <option>-- Select Quotation --</option>
            @php
            $i=1;
            @endphp
            @foreach($quotation_details_revised as $revised)
            <option value="{{$revised['id'].'/'.$quotation_details['id']}}">Revise {{$i}}, {{date('d-m-Y H:i A',strtotime($revised['revised_date']))}}</option>
            @php
            $i++;
            @endphp
            @endforeach
  ...
  <div class="page-content container-fluid">
    <div class="row justify-content-center"> @if (\Session::has('success'))
      ...
      @endif
      <div class="col-md-12">
            <form class="" action="{{ url('admin/quotation-edit/'.$quotation_details['_id']) }}" id='invoiceform' method="post" autocomplete="on" onsubmit="return validate();">
              @csrf
              <div class="card">
                <div class="card-body">
  ...
  <div class="col-lg-4 col-md-4 col-sm-12">
    ...
    <div class="row form-three-box">
      ...
      <div class="col-lg-4 col-md-4 col-sm-12">
        <label>Type</label>
        <input type="text" autocomplete="off" readonly value="{{ $quotation_details['quotation_type']}}" class="form-control">
      </div>
      <div class="col-lg-4 col-md-4 col-sm-12 ">
        <label>SrNo.</label>
  ...
  <div class="col-lg-4 col-md-4 col-sm-12">
    <label>Qu. Date</label>
      <input type="text" autocomplete="off" readonly id="quotation_date" value="{{globalDateformatNet($quotation_details['quotation_date'])}}" required class="form-control small_field_custom" name="quotation_date">
  </div>
  <div class="col-lg-4 col-md-4 col-sm-12 ">
    <label>Due Date</label>
      <input type="text" autocomplete="off" readonly id="quotation_due_date" value="@if($quotation_details['quotation_due_date']!='1970-01-01'){{ globalDateformatNet($quotation_details['quotation_due_date'])}} @endif" class="form-control small_field_custom" name="quotation_due_date">
  </div>
  <div class="col-lg-4 col-md-4 col-sm-12 ">
    <label>Ref. Date</label>
      <input type="text" autocomplete="off" readonly id="quotation_ref_date" value="@if($quotation_details['quotation_ref_date']!='1970-01-01'){{ globalDateformatNet($quotation_details['quotation_ref_date'])}} @endif" class="form-control small_field_custom" name="quotation_ref_date">
  </div>
</div>
<div class="form-new-box">
                      <label>Customer Ref No.</label>
      <input name="quotation_ref_no" readonly required autocomplete="off" value="{{$quotation_details['quotation_ref_no']}}" class="form-control" type="text" >
                    </div>
  ...
  <div class="col-lg-4 col-md-4 col-sm-12 ">
    <div class="form-new-box">
      <label>Remarks</label>
      <textarea name="quotation_remarks" readonly class="form-control">{{$quotation_details['quotation_remarks']}}</textarea>
    </div>
    
    <div class="row form-two-box">
  <div class="col-lg-6 col-md-6 col-sm-12 ">
    <label>Priority</label>
    <input type="text" autocomplete="off" readonly value="{{ $quotation_details['quotation_priority']}}" class="form-control">
  </div>
  <div class="col-lg-6 col-md-6 col-sm-12 ">
    <label>Status</label>
    <input readonly required autocomplete="off" value="{{$quotation_details['quotation_status']}}" class="form-control" type="text" >
  </div>
</div>
  </div>
</div>
                  ...
                  <div class="purchaseorder-box quotationcreate-box-wrap">
                    ...
                    <div class="table-wrap big-table">
                      <table class="editable-table table table-striped quotationcreate-table left-fix-table" id="editableTable1">
						...
						<input type="text" value="{{$item_details['comment']}}" name="row[{{$i}}][comment]" class="autocomplete-dynamic form-control" id="comment_rel_{{$i}}" />
                            </td>
                            <td data-text="Item Description">						  
						<input type="text" value="{{$item_details['description']}}" name="row[{{$i}}][description]" class="autocomplete-dynamic form-control" id="description_rel_{{$i}}" />
                            </td>
						...
						<input type="text" readonly value="{{$item_details['vendor_sku']}}" name="row[{{$i}}][vendor_sku]" class="autocomplete-dynamic form-control" id="vendor_sku_rel_{{$i}}" required />
                          ...
                          <td data-text="Tax">
                              <input type="text" readonly value="{{$item_details['tax_rate']}}" name="row[{{$i}}][tax_rate]" cus="{{$i}}" class="autocomplete-dynamic tax_rate form-control" id="tax_rate_rel_{{$i}}" required />
                              </td>
                          ...
                          <td data-text="List Price">
                            ...
                            <td data-text="Rate">
                              <input type="text" readonly value="{{$item_details['rate']}}" name="row[{{$i}}][rate]" cus="{{$i}}" class="autocomplete-dynamic item-rate form-control" id="rate_rel_{{$i}}" />
                          </td>
                          <td data-text="Stock">
                              <input type="text" readonly value="{{$item_details['stock']}}" readonly name="row[{{$i}}][stock]" class="autocomplete-dynamic form-control" id="stock_rel_{{$i}}" /> 
                              </td>
                          <td data-text="MRP">
                            <input type="text" readonly value="{{$item_details['mrp']}}" name="row[{{$i}}][mrp]" cus="{{$i}}" class="autocomplete-dynamic form-control mrp" id="mrp_rel_{{$i}}" /> 
                             ...
                             <td data-text="Dis %">
                              <input type="number" readonly value="{{$item_details['discount_per']}}" name="row[{{$i}}][discount_per]" cus="{{$i}}" class="autocomplete-dynamic item-disc form-control" id="discount_per_rel_{{$i}}" />
                          </td>
                          ...
                          <td data-text="Quantity">
                              <input type="number" readonly value="{{$item_details['quantity']}}" name="row[{{$i}}][quantity]" cus="{{$i}}" class="autocomplete-dynamic ss-qty form-control" id="quantity_rel_{{$i}}" />
                              </td>
        ...
        <div class="row quotationcreate-wrap">
            ...
            <div class="table-box">
              ...
              <thead>
                ...
                <tr id='san-{{$key}}'>
                    ...
                    <td>&nbsp;</td>
                    <input type='hidden' name='other_charges_name[]' value='{{$charges['other_charges_name']}}'><input type='hidden' name='other_charges_val[]' value="{{$charges['other_charges_val'][$key]}}" >
                </tr>
                @php
          ...
          <div class="col-lg-3 col-md-3 form-new-box-wrap">
            ...
            <div class="form-new-box">
              <label>Name</label>
              <input type="text" readonly value="{{$quotation_details['quotation_contact_name']}}" required name="quotation_contact_name" class="form-control">
            </div>
            <div class="form-new-box">
              <label>Department</label>
              <input type="text" readonly value="{{$quotation_details['quotation_department_name']}}" required name="quotation_department_name" class="form-control">
            </div>
            <div class="form-new-box">
              <label>Phone</label>
              <input type="text" readonly value="{{$quotation_details['quotation_contact_phone']}}" required name="quotation_contact_phone" class="form-control">
            </div>
            <div class="form-new-box">
              <label>Email</label>
              <input type="email" readonly value="{{$quotation_details['quotation_contact_email']}}" required name="quotation_contact_email" class="form-control">
            </div>
          </div>
          <div class="col-lg-3 col-md-3 form-new-box-wrap">
            <h2>Totals</h2>
            ...
            <div class="form-new-box">
              ...
              <input type="text" readonly autocomplete="off" value="{{$quotation_details['quotation_saletax']}}" name="quotation_saletax" id="sale-tax" class="form-control">
            </div>
            <div class="form-new-box">
          ...
          <div class="col-lg-12 quotationcreate-button-wrap">
            ...
            <a href="{{route('quotation_list')}}" class="btn btn-primary btn-data-save waves-effect waves-classic">Back</a>
...
<script>
    $(function(){
       $(document).on('change','#quotation_revised',function(){
          var id = $(this).val();
          var url = '{{url("admin/quotation-revised/")}}';
          window.open(url+'/'+id, 'Quotation'); 
       }); 
    });
</script>
