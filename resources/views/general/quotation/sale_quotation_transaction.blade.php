...
@section('pageTitle', 'Quotation Transaction')
@section('pagecontent')
    ...
    <div class="new-design-inner">
      <div class="new-header-div-fot-btn search-design-data">
        <div class="page-header">
          <h1 class="page-title">Quotation Transaction</h1>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item">Quotation Transaction</li>
          </ol>
        </div>
        <div class="payment-heading-box1">
        <form method="POST" action="{{url('admin/quotation_transaction')}}" class="custom-inline-form">
          @csrf
          <div class="form-box">
          ...
          <div class="form-box">
              <input class="form-control" type="text" name="quotation_no" value="{{$request->quotation_no}}" autocomplete="off" placeholder="Qu. No">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" name="quotation_ref_no" value="{{$request->quotation_ref_no}}" autocomplete="off" placeholder="Qu. Ref No.">
            </div>
            <div class="form-box">
              <input class="form-control" type="text" id="quotation_date" name="quotation_ref_date" value="{{$request->quotation_ref_date}}" autocomplete="off" placeholder="Qu. Date">
            </div>
          ...
          <div class="form-box form-box-submit"><a href="{{route('quotation_transaction')}}" class="btn btn-primary waves-effect waves-classic" title="Reload" name="reload"><i class="icon md-refresh" aria-hidden="true"></i></a></div><div class="form-box  form-box-submit">
          ...
          <div class="panel-body">
            <table class="table table-bordered t1">
              <thead>
                ...
                <th class="bg-info" id="location" style="color:#fff !important;">Quotation No.</th>
              ...
              <tbody id="fb_table_body">
                  ...
                  @php
                  $get_quotation_item = getQuotationItemDetails($quo['quotation_id']);
                  //pr($get_quotation_item);
                  @endphp
                  <tr>
                  <td>{{$cus_data[$quo['customer_id']]}}</td>
                  <td>{{$get_quotation_item['quotation_no']}}</td>
                  <td>{{$get_quotation_item['quotation_ref_no']}}</td>
                  <td>@if($get_quotation_item['quotation_ref_date']!='1970-01-01'){{ globalDateformatNet($get_quotation_item['quotation_ref_date'])}} @endif</td>
                  <td>{{$quo['item_name']}}</td>
                  <td>{{$quo['quantity']}}</td>
                  </tr>
                  @endforeach
