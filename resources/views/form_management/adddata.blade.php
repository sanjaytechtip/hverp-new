...
@endphp
@if($adddata['id']=='617f8983d0490476da34a012')
<style>
...
<div class="page formbuilder">
	<div class="page-header">
        ...
        <ol class="breadcrumb">
			...
			<li class="breadcrumb-item"><a href="{{ route('listdata',$adddata['id']) }}">{{$adddata['form_name']}} : List</a></li>
...
$(function(){
  ...
  $(document).on('blur change','#mrp',function(){
  var mrp_val = $(this).val();
  var rate_val = $('#net_rate').val();
  if(mrp_val!='' && rate_val!='')
  {
    if(eval(mrp_val)<=eval(rate_val))
    {
      alert("MRP price is always greater then Net Rate.");
      $('#mrp').val('');
      return false;
    }
  }
...
<script>
$(function(){
  $(document).on('blur change','#net_rate',function(){
  var mrp_val = $('#mrp').val();
...
<script>
$(function(){
    $("input[name=eud][value='no']").attr("checked",true);
    if($("input[name=eud]:checked").val()=='no')
    {
    $('.eud_file').hide();
    }else{
    $('.eud_file').show();   
    ...
    $('.eud input:radio').click(function() {
    if ($(this).val() === 'yes') {
        $('.eud_file').show();
    } else if ($(this).val() === 'no') {
        $('.eud_file').hide();
